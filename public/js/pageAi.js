document.addEventListener('DOMContentLoaded', () => {
  ...
  const els = {
    ...
    includeVideo: document.getElementById('ai-include-video'), // ✅ NEW
  ...
  profileEls.saveBtn.onclick = async () => {
    try {
      ...
      await fetch(`/api/ai/page/${pageId}/profile`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  els.topicSelect.onchange = () => {
    ...
    els.includeVideo.checked = t.includeVideo === true; // ✅ NEW
  };
  ...
  els.editBtn.onclick = async () => {
    ...
    try {
      const payload = {
        ...
        includeVideo: els.includeVideo.checked // ✅ NEW
      };
  ...
  async function loadUpcomingPosts() {
    try {
      ...
      posts.forEach(p => {
        let mediaHtml = '';
        if (p.mediaUrl) {
          // Detect video: ends with .mp4 or contains '/video/upload/'
          const isVideo = p.mediaUrl.endsWith('.mp4') || p.mediaUrl.includes('/video/upload/');
          if (isVideo) {
            mediaHtml = `<video width="120" controls>
                           <source src="${p.mediaUrl}" type="video/mp4">
                           Your browser does not support video.
                         </video>`;
          } else {
            mediaHtml = `<a href="${p.mediaUrl}" target="_blank">
                           <img src="${p.mediaUrl}" width="80" style="border-radius:4px">
                         </a>`;
          }
        } else {
          mediaHtml = '—';
